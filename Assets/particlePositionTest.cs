...
public class particlePositionTest : MonoBehaviour
{
    ...
    private Vector3 particleEmissionPosition;
    ...
    void Update()
    {
        ...
        if(handModel != null && handModel.IsTracked){
            ...
            foreach (var finger in fingers)
                {
                    particleEmissionPosition += finger.TipPosition.ToVector3();
                };
            
            particleEmissionPosition = new Vector3(particleEmissionPosition.x,particleEmissionPosition.y,particleEmissionPosition.z)/5;
    
            // Set extensionFactor equal to the relative distance between fingers and centre
            foreach (var finger in fingers){
                extensionFactor += Vector3.Distance(finger.TipPosition.ToVector3(),particleEmissionPosition);
            };
            ...
            debugSphere.transform.position = particleEmissionPosition;
            ...
            //Debug.DrawRay(particleEmissionPosition,palm.transform.position-particleEmissionPosition);
            // Draw a ray from each finger's TipPosition to the central position
            foreach (var finger in fingers)
            {
                Debug.DrawRay(particleEmissionPosition, finger.TipPosition.ToVector3()-particleEmissionPosition);
            };
            ...
            transform.position = particleEmissionPosition;
            ...
            //Debug.Log(radiusValue);
                float normal = Mathf.InverseLerp(0f, 0.16f, extensionFactor);
            // Debug.Log(normal);
                float scaledValue = Mathf.Lerp(0f, 1f, normal);
                float radiusValue = (logCurve.Evaluate(scaledValue))*40;
